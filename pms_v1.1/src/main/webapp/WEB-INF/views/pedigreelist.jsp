	...
	<link rel="StyleSheet" href="css/modal.css" type="text/css" />
		...
		<script type="text/javascript">
			...
			<c:forEach items="${pedigrees}" var="pedigree">
				...
				pedigree.validationStatus = "${pedigree.validationStatus}";
				pedigree.modifiedTime = "${pedigree.modifiedTime}";
				pedigree.xml = '${pedigree.xml}';
				pedigrees.push(pedigree);
			</c:forEach>
			
			// view로 넘어온 trace 정보를 저장
			var traceInfoList = new Array();
			<c:forEach items="${traceInfoList}" var="tracePath">
				var tracePath = new Object();
				tracePath.sgtin = "${tracePath.sgtin}";
				tracePath.path = new Array();
				<c:forEach items="${tracePath.path}" var="node">
					var info = new Object();
					info.company = "${node.company}";
					info.type = "${node.type}";
					info.eventTime = "${node.eventTime}";
					info.validity = "${node.validity}";
					tracePath.path.push(info);
				</c:forEach>
				traceInfoList.push(tracePath);
			</c:forEach>

			$(document).ready(function() {
				var pedigreeXml = "";
				// pedigree 정보를 표 형태로 표시
				function refresh_pedigree() {
					...
					// 테이블의 각 엔트리를 pedigeee 로부터 정보를 얻어서 표시
						...
						if(i<10) {
							...
							for (var j = 0; j < 4; j++) {
								...
								if (j == 0)
									...
									$(td2).text(pedigree_element.validationStatus);
								else if (j == 3)
							...
							$(tr).click(function() {
		                    	...
		                    	// pedigree trace 정보 구조 설정
		                    	for (var i = 0; i < traceInfoList.length; i++) {
									var selectedSgtin = $(this).attr('class').split(" ")[0]; 
									if (selectedSgtin == traceInfoList[i].sgtin) {
										console.log(traceInfoList[i])
										set_trace_info(traceInfoList[i].path);
										break;
									}
								}
							});
		                    ...
		                    $(tr).addClass(pedigree_element.validationStatus);
		                    setPedigreeColor(tr);
						}
					}
				}
				...
				// trace 정보 설정, 표 형태로 구성하며 회사명, 타입, 생성시간, 검증결과를 표시한다.
				function set_trace_info(traceInfo) {
					$("#traceInfo").empty();
					var tr = document.createElement("tr");
					$("#traceInfo").append(tr);
				...
				function setPedigreeColor(tr) {
					...
					else if($(tr).is('.failure')) {$(tr).css("background-color", "#ff8888");}
...
<div class="container" id="step0">
	<div class="row">
		<div class="text-center col-sm-12">
			...
			<div class="panel panel-default">
				<div class="panel-heading">저장된 Pedigree 리스트</div>
					...
					<form class="form-horizontal" name="f" method="post">
						<div class="form-group">
							<div>
								...
								<a id="btn_initial_info_modal" class="btn btn-primary" style="position: relative; margin-left: 25px; float: inherit; width: 25%;">Initial Pedigree 생성</a>
								...
								<!-- 신뢰하는 인증서 목록이 변화되었을 경우 pedigree 재검증을 수행함으로써 pedigree의 유효성 재검사-->
								<input id="revalidate" class="btn btn-primary" name="submit" type="submit" formaction="${homeUrl}/revalidate" value="유효성 재검증" style="position: relative; margin-left: 25px; float: inherit; width: 25%;">
								<!-- pedigree export를 위한 modal을 호출 -->
								<a id="btn_partner_info_modal" class="btn btn-primary" style="position: relative; margin-left: 25px; float: inherit; width: 25%;">Pedigree 전달</a>
							</div>
						</div>
					</form>						
				</div>
	...
	<div class="row">
		<div class="text-center col-sm-6">
			...
			<div class="panel panel-default">
				<div class="panel-heading">pedigree 내 제품 정보</div>
		...
		<div class="text-center col-sm-6">
			<!-- 선택한 pedigree의 trace정보를 표로 표시 -->
			<div class="panel panel-default">
				<div class="panel-heading">pedigree 구조 및 검증 정보</div>
				<div class="panel-body">
					<div class="panel-body text-left">
							...
							<div class="panel panel-default">
								...
								<div class="panel-body">
									<form class="form-horizontal" name="f" method="post" accept-charset="UTF-8" onsubmit="document.charset='UTF-8'">
										<div class="form-group">
											<label for="inputSerialNumber" class="col-sm-3 control-label">sgtin</label>
											<div class="col-sm-6">
												<input type="text" class="form-control" id="gtin" name="serialNumber"
													placeholder="urn:epc:id:sgcn:0614141.11111.12345">
											</div>
										</div>
										<div class="form-group">
