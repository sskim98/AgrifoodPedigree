...
import kaist.gs1.pms.initialPedigree.PartnerIdValueTypeType;
...
public class shippedPedigree {
	...
	// EPCIS event로부터 shipped pedigree를 생성하는 루틴, 이 루틴은 아래 루틴으로 변경
	shippedPedigree(String sgtin, InfoType_Product product, InfoType_Company companyInfo, InfoType_Partner partnerInfo) {
	...
	shippedPedigree(String sgtin, InfoType_Product product, Document eventDoc, InfoType_Company companyInfo, InfoType_Partner partnerInfo) {
		//Document eventDoc = buildDocumentfromString(eventXml);
		// sgtin에서 serial 추출
		this.id = sgtin;
		String[] array = sgtin.split("\\.");
